...
/**
 * implementation of {@link TimeServer} that uses a singleton linked list. With
 * the enqueue decides when to add agents {@link Agent} to the list.the list
 * will never be empty.
 *
 */
public final class TimeServerLinked extends Observable implements TimeServer {
	private static final class Node {
		...
		public Node(double waketime, Agent agent, Node next) {
			this.waketime = waketime;// when the next will be executed
			this.agent = agent; // low wake time begining and big in end
			this.next = next;
		}
	}
