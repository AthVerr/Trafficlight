...
/**
 * A swing implementation of {@link Animator}, using a {@link JFrame} to display
 * the animation. The {@link JFrame} is created and displayed by the
 * constructor.
 * 
 * Calls to <code>update()</code> result in a call to
 * <code>painter.paint()</code>. This is executed in the swing thread while the
 * main thread is paused for <code>delay</code> milliseconds.
 */
public class SwingAnimator implements Animator {
	 ...
	 * 
	 * @param name
	 *            The name to be displayed on the graphical window.
	 * @param width
	 *            The width of the display, in pixels.
	 * @param height
	 *            The height of the display, in pixels.
	 * @param delay
	 *            Time to pause after an update, in milliseconds.
	 */
	public SwingAnimator(final SwingAnimatorPainter painter, final String name, final int width, final int height,
			int delay) {
		_delay = delay;
		// Create a graphics window and display it
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				_content = new ContentPane(painter, width, height); // A
																	// paintable
																	// component
																	// for
																	// content
				...
				_frame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE); // End
																				// program
																				// if
																				// Frame
																				// is
																				// closed
				_frame.setContentPane(_content); // Associate the content with
													// the Frame
	...
	/**
	 * Calls to <code>update</code> are executed in the swing thread, while the
	 * main thread is paused for <code>delay</code> milliseconds.
	 */
	...
	/**
	 * A component for painting. All code is executed in the swing thread.
	 */
